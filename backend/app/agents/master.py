...
"""
    ...
    raw = call_ollama(system_prompt, f"Query: {query}", temperature=0.3)
...
"""
    ...
    raw = call_ollama(system_prompt, user_prompt, temperature=0.25)

    try:
        synthesis = safe_json_load(raw)
    except Exception:
        strict_prompt = system_prompt + "\n\nIMPORTANT: Output ONLY raw JSON. No text."
        raw_retry = call_ollama(strict_prompt, user_prompt, temperature=0.1)
